...
function App() {
  ...
  const stockAdditionHandler = (productName) => {
  ...
  const stockReductionHandler = (productName) => {
    const productToUpdate = {...products.find(
      (item) => productName === item.name
    )}
  ...
  const removeFromCart = (productToRemove) => {
    ...
    console.log(removeProductsInCart + ' hasdgyuawg')
    setCart(removeProductsInCart) 
  };  

  const StockResetHandler = (product) => {
    const productInCart = cart.find((element) => product.id === element.id)

    let stockToUpdate = {...products.find(
      (element) => product.id === element.stock_id)}
    if (stockToUpdate.stock) {
      stockToUpdate.stock =+ productInCart.quantity 
    } else {
      stockToUpdate = productInCart
    }

    console.log(productInCart + ' idx')
    console.log(stockToUpdate + ' invntryStock')
    const newProducts = products.filter(
      (item) => item.name !== product.name
    )
    console.log(newProducts +' newProd')
    setProducts([...newProducts, stockToUpdate])
};  
